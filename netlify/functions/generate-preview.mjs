...
const extraInfo = (d) => d.extraDetails ? ` The parent also shared these extra details about the child: "${d.extraDetails}". Weave as many of these details into the story as naturally as possible — they are the things that will make the child feel like this story truly knows them.` : '';

const STORY_PROMPTS = {
  bedtime: (d) => `Write a beautiful, deeply personal bedtime story for a ${d.age} year old ${d.gender} named ${d.childName}. Their best friend is called ${d.friendName}. They love ${d.interest}. ${d.proudOf ? `Something they are really proud of recently: ${d.proudOf}. Weave this in naturally as a moment of triumph.` : ''} ${d.hasPet ? `They have a beloved pet called ${d.petName} who plays a small sweet role in the story.` : ''}${extraInfo(d)} Use ${d.childName}'s name throughout the story naturally, at least 8 times. The story must be warm, safe, soothing, and end with ${d.childName} drifting off to sleep feeling completely safe and deeply loved. Write approximately ${WORD_COUNTS[d.length] || 600} words. Do not include chapter titles or headings. Start the story immediately with no preamble.`,

  journey: (d) => `Write a gripping, exciting adventure story for a ${d.age} year old ${d.gender} named ${d.childName}. Their best friend ${d.friendName} is right by their side through every twist and turn. They love ${d.interest}. ${d.proudOf ? `Recently they ${d.proudOf}, which gives them the confidence to face this adventure.` : ''} ${d.hasPet ? `Their loyal pet ${d.petName} saves the day at a critical moment in the story.` : ''}${extraInfo(d)} Structure as ${d.length === 'epic' ? '5 short chapters' : d.length === 'standard' ? '3 short chapters' : '2 short chapters'}, each ending on an exciting cliffhanger that makes them desperate to hear what happens next. Perfect for reading aloud on a long journey. Write approximately ${WORD_COUNTS[d.length] || 600} words. Start the story immediately.`,

  learning: (d) => `Write an exciting, educational adventure story for a ${d.age} year old ${d.gender} named ${d.childName}. Their trusty sidekick is ${d.friendName}. They love ${d.interest}. ${d.hasPet ? `Their pet ${d.petName} has a special ability that helps solve one of the challenges.` : ''}${extraInfo(d)} ${d.childName} is a superhero whose incredible power is ${d.subject}. A villain threatens the world and can only be defeated by solving ${d.subject} challenges that are genuinely educational and age-appropriate for a ${d.age} year old. Make the learning feel natural and exciting, not like a lesson. Write approximately ${WORD_COUNTS[d.length] || 600} words. Start the story immediately.`,

  custom: (d) => `Write a warm, personalised story for a ${d.age} year old ${d.gender} named ${d.childName}. Their best friend is ${d.friendName}. They love ${d.interest}. ${d.proudOf ? `Something they are proud of recently: ${d.proudOf}. Weave this in naturally.` : ''} ${d.hasPet ? `Their pet ${d.petName} plays a meaningful role in the story.` : ''}${extraInfo(d)} Here is the situation the parent wants this story to address: "${d.customScenario}". Write a story that helps ${d.childName} feel prepared, brave, calm, or excited about this situation. The story should be age-appropriate, use ${d.childName}'s name at least 8 times, and weave the scenario into a narrative where the child feels empowered and positive by the end. Do not lecture or moralise. Let the story do the emotional work naturally. Write approximately ${WORD_COUNTS[d.length] || 600} words. Start the story immediately.`
};
