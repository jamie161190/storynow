...
// It generates the story, saves partial result, generates TTS,
// then saves the complete result for polling to pick up.
// ============================================================
export const handler = async (event) => {
  ...
  try {
    ...
    console.log('[BG] Starting story generation for job:', jobId, 'category:', storyData.category);

    // ── Generate story with Anthropic ──
    ...
    try {
      ...
      fullStory = storyText;
      console.log('[BG] Story generated in', Date.now() - startTime, 'ms, words:', fullStory.split(' ').length);
    } catch (apiErr) {
      console.error('[BG] Anthropic API error after', Date.now() - startTime, 'ms:', apiErr.message);
      // Save error state so polling can report it
    ...
    // ── STAGE 1: Save story text immediately ──
    if (jobId && supabaseUrl && supabaseKey) {
      try {
        ...
        console.log('[BG] Saved partial result (story text) for job:', jobId);
      } catch (saveErr) {
        console.error('[BG] Failed to save partial result:', saveErr.message);
      }
    }
    ...
    if (!ttsResponse.ok) {
      ...
      // Story is already saved as partial, check-preview will recover with its own TTS attempt
      return { statusCode: 200 };
    }
    ...
    // ── STAGE 2: Save complete result (story + audio) ──
