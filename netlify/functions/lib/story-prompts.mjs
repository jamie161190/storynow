...
- household.pet: legacy single-pet field. Use it if present and household.pets is empty. Same rules as pets below.
- household.pets: array of pets. Each is rendered as {name, sketch}. Use the names. DO NOT reintroduce a breed or species label unless the sketch itself mentions it. "Nova" is Nova. Not "Nova the border collie." When multiple pets are present, give each its own beat — a moment of action, presence, or interaction with a child. Don't merge them into "the pets" or "the dog and the cat" — they're separate characters with separate names. A second pet that only exists in narration but never DOES anything has been wasted.
