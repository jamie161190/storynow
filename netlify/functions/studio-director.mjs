...
- Over-edit. Raw feels more real than polished.
...
export default async (req) => {
  ...
  // ── Analyse uploaded clips/photos ──
  if (action === 'analyse') {
    return handleAnalyse(apiKey, body);
  }

  // ── Generate structured ad builds from analysed assets ──
  if (action === 'concepts') {
...
async function handleAnalyse(apiKey, { assets, userNotes }) {
  ...
  const hasLongClips = assets.some(a => a.isLong);
...
- moments: an array of the best 3-5 moments with:
  - timestamp: "M:SS" format
  - time: seconds as number
  - startTime: suggested clip start (seconds)
  - endTime: suggested clip end (seconds)
...
Return ONLY the JSON array, no other text.`
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('[DIRECTOR] Claude Vision error:', response.status, err);
      return json({ error: 'Analysis failed. Try again.' }, 500);
    }
    ...
    try {
      ...
      const analyses = jsonMatch ? JSON.parse(jsonMatch[0]) : [];
      return json({ analyses });
    } catch (e) {
      console.error('[DIRECTOR] JSON parse error:', e.message);
      return json({ analyses: [], raw: text });
    }
  } catch (e) {
    console.error('[DIRECTOR] Analysis error:', e.message);
  ...
  const assetsDesc = analyses.map((a, i) => {
    ...
    if (a.moments && a.moments.length) {
      ...
      for (const m of a.moments) {
        desc += `\n   - ${m.timestamp} "${m.name}" [${m.usability}] ${m.description} (${m.marketingPotential}/5)`;
      }
    }
    return desc;
  }).join('\n');
...
${platform ? `Target platform: ${platform}` : 'Create concepts for multiple platforms (TikTok, Meta, YouTube).'}

Generate exactly 3 complete ad concepts. Each concept MUST include a structured JSON timeline that can be directly loaded into our Compose tool.

For each concept, return a JSON object in this EXACT format:
  ...
  "concepts": [
    {
      ...
      "format": "9:16",
      "duration": 22,
      "whyItWorks": "One line explaining why this will perform",
      "music": "bedtime",
      "hookText": "The text that appears on screen in first 3 seconds",
      "adType": "emotional",
      "timeline": [
        {
          "type": "text",
          "duration": 3,
          "config": {
            "text": "She had no idea...",
            "style": "hook",
            "subtitle": "what was about to happen"
          }
        },
        {
          "type": "clip",
          "duration": 5,
          "config": {
            "assetName": "mum-pressing-play-reaction",
            "trimStart": 0,
            "trimEnd": 5,
            "textOverlays": [
              { "text": "First time hearing her daughter's name", "position": "bottom", "size": 24, "color": "#FFFFFF" }
            ]
          }
        },
        {
          ...
          "duration": 8,
          "config": {
            "sequenceId": "create-story",
            "screens": ["child-details", "loading", "preview-player"]
          }
        },
        {
          "type": "clip",
          "duration": 4,
          "config": {
            "assetName": "child-amazed-face",
            "trimStart": 2,
            "trimEnd": 6,
            "textOverlays": [
              { "text": "Her face when she heard her name", "position": "top", "size": 20, "color": "#FFFFFF" }
            ]
          }
        },
        {
...
RULES FOR BUILDING TIMELINES:
1. Every ad starts with either a "text" hook card or a "clip" with a scroll-stopping moment
2. Every ad ends with an "endcard" block
3. Use "sequence" blocks to show the product (mockup screens). You can specify which specific screens to show.
4. Use "clip" blocks for real footage. Reference assets by their exact "name" from the analysis.
5. Add textOverlays to clips to add context and make moments more powerful
6. For clips from LONG VIDEO assets with moments, use the moment's startTime/endTime as trimStart/trimEnd
7. Mix real footage with mockup sequences. The best ads alternate between emotion (clips) and explanation (mockups).
8. Text card styles: "hook" (bold, attention-grabbing), "cta" (clean call to action), "testimonial" (quote style)
9. Each concept should be a genuinely different AD TYPE. Make one emotional, one product-demo focused, one hook-driven or gift-angle.
10. Keep TikTok ads 15-25s, Meta 20-45s, YouTube 30-60s
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: 'claude-sonnet-4-6',
        max_tokens: 8192,
        system: CONCEPTS_BRAIN,
        messages: [{ role: 'user', content: userPrompt }]
      })
    });

    if (!response.ok) {
      return json({ error: 'Concept generation failed' }, 500);
    }
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        return json({ concepts: parsed.concepts, raw: content });
      }
      // Fallback: try parsing as markdown concepts
      const concepts = parseFallbackConcepts(content);
      return json({ concepts, raw: content });
    } catch (e) {
      console.error('[DIRECTOR] Concepts JSON parse error:', e.message);
      // Fallback to text parsing
      const concepts = parseFallbackConcepts(content);
      return json({ concepts, raw: content });
    }
  } catch (e) {
    console.error('[DIRECTOR] Concepts error:', e.message);
    return json({ error: 'Concept generation failed: ' + e.message }, 500);
  }
}

// Fallback parser for when Claude returns markdown instead of JSON
function parseFallbackConcepts(text) {
  const sections = text.split(/(?=## Concept \d)/g).filter(s => s.includes('## Concept'));
  return sections.map((section, i) => {
    const lines = section.split('\n');
    const name = (lines[0] || '').replace(/^## Concept \d+:\s*/, '').trim();
    ...
    const timelineLines = lines.filter(l => /^\s*-\s*\d/.test(l));
    const timeline = timelineLines.map(l => l.replace(/^\s*-\s*/, '').trim());
    ...
    return {
      ...
      whyItWorks: get('Why it works'),
      music: get('Music'),
      hookText: get('Opening hook'),
      timeline,
      raw: section
    };
  });
...
export const config = { path: '/api/studio-director' };
