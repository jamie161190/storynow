// ONE-TIME USE: Generate 3 homepage sample narrations
// Hit: curl https://storytold.ai/api/generate-samples
...
export default async (req) => {
  ...
  for (const sample of samples) {
    try {
      console.log('Generating ' + sample.name + ' with voice ' + sample.voice + '...');
      ...
      if (!ttsRes.ok) {
        ...
        console.error(sample.name + ' TTS error:', ttsRes.status, err);
        results.push({ name: sample.name, error: 'TTS failed: ' + ttsRes.status });
        continue;
      }

      const audioBuffer = await ttsRes.arrayBuffer();
      console.log(sample.name + ' audio generated:', audioBuffer.byteLength, 'bytes');
      ...
      if (supabaseUrl && supabaseKey) {
        ...
        console.log(sample.name + ' upload:', uploadRes.status);

        const publicUrl = supabaseUrl + '/storage/v1/object/public/stories/' + sample.file;
        results.push({ name: sample.name, bytes: audioBuffer.byteLength, publicUrl, upload: uploadRes.status });
      } else {
        results.push({ name: sample.name, bytes: audioBuffer.byteLength, error: 'No Supabase config' });
      }
    } catch (err) {
      console.error(sample.name + ' error:', err.message);
      results.push({ name: sample.name, error: err.message });
    }
  }
